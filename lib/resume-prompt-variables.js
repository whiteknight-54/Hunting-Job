 ...
 * Optional application questions are appended to job context only.
 * Role title and company name are not injected here (PDF filename only).
 */
export const buildAtsSubstitutionVariables = (profileData, { jobDescription, questions = "" }) => {
  ...
  return {
    ...
    resumeTitle: profileData.title || profileData.name || "Professional",
  };
};
