...
describe("career context", () => {
  ...
  it("includes chronology rules in career guidance", () => {
    const ctx = buildCareerContext(profileData);
    assert.match(ctx.careerGuidance, /WORK-HISTORY WRITING RULES/);
...
describe("ATS prompt composition", () => {
  it("composes career-customized INPUT, GLOSSARY, instructions, and OUTPUT", async () => {
    ...
    assert.match(blocks.input, /Work history/);
    assert.doesNotMatch(blocks.input, /CAREER PATH/);
  ...
  it("OUTPUT example JSON has one experience entry per profile role", async () => {
    ...
    assert.match(blocks.instructions, /WORK-HISTORY WRITING RULES/);
  });
});
