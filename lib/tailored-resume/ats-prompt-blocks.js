...
--- Career path (profile-derived — anchor all experience[].details to this arc) ---
${vars.careerPath}
...
**Terms:**
- work history = INPUT experience[] rows (company | title | location | dates)
- job context = job description + optional application questions in INPUT
- career path = profile-derived progression in INPUT — all detail strings must stay credible on this arc
...
export function buildAtsOutputBlock(vars) {
  ...
  const exampleJson = {
    ...
    summary:
      "Senior professional paragraph tailored to job context and career path. Qualitative outcomes only unless QA/ML exceptions apply.",
    skills: {
...
- Root keys ONLY: ${TAILORED_RESUME_ROOT_KEYS.join(", ")}.
- Exactly ${vars.experienceCount} experience objects — same order as INPUT work history / career path.
