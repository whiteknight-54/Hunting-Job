...
import { allQuickCopyIconUrls, preloadQuickCopyIcons } from "./preload-quick-copy-icons.js";
import {
  clearProfileOverride,
  readProfileOverride,
  writeProfileOverride,
} from "./profile-override-storage.js";
...
export function useProfileSession() {
    ...
    const load = async () => {
      try {
        ...
        startTransition(() => {
          ...
          setProfileOverride(readProfileOverride(profileSlug));
  ...
  const runProfileMigration = async () => {
    ...
    try {
      ...
      // if (githubProfilesUrl) {
      //   window.open(githubProfilesUrl, "_blank", "noopener,noreferrer");
      // }
    } catch (err) {
      console.error("Add new on GitHub:", err);
    } finally {
  ...
  useEffect(() => {
    const effective = profileOverride ?? baseProfileData;
    if (!effective) return;
    preloadQuickCopyIcons(allQuickCopyIconUrls());
  }, [profileOverride, baseProfileData]);

  const profileData = profileOverride ?? baseProfileData;
  const hasProfileOverride = profileOverride != null;

  const saveProfileOverride = (data) => {
    if (!profileSlug || !data) return;
    writeProfileOverride(profileSlug, data);
    setProfileOverride(data);
  };

  const resetProfileOverride = () => {
    if (!profileSlug) return;
    clearProfileOverride(profileSlug);
    setProfileOverride(null);
  };
