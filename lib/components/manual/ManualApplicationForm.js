  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 280px), 1fr))", gap: 12, marginBottom: 16 }}>
        <div>
          <label style={labelStyle}>Role title (PDF filename)</label>
          <input
            type="text"
            value={roleName}
        ...
        <div>
          <label style={labelStyle}>Company name (PDF filename)</label>
          <input
            type="text"
            value={companyName}
            onChange={(e) => setCompanyName(e.target.value)}
            placeholder="e.g. Acme Corp — used only when downloading the PDF"
            style={{
              ...inputStyle,
