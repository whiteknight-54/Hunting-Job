    ...
    <div style={cardStyle}>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}>
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
              <button
                ...
                style={primaryBtn(disable || !roleName.trim() || !companyName.trim() || !pastedContent.trim())}
              >
                {disable ? "Generating…" : "Download resume PDF"}
              </button>
              <select
              ...
              <div style={{ marginTop: 10, marginBottom: 4 }}>
                ...
                {disable ? (
                  <div style={{ fontSize: 12, lineHeight: 1.45, color: colors.text, opacity: 0.92 }}>
                    <strong style={{ color: colors.accent }}>Generating PDF…</strong>
                    <span style={{ opacity: 0.75 }}> · </span>
                    <strong style={{ color: colors.accent }}>{elapsedTime}s</strong>
                  </div>
                ) : (
                  <div style={{ fontSize: 12, lineHeight: 1.45 }}>
                    <strong style={{ color: colors.errorText }}>PDF generation failed</strong>
                    <span style={{ color: colors.errorText, opacity: 0.88 }}> · after {elapsedTime}s</span>
                    <div style={{ marginTop: 6, fontSize: 12, lineHeight: 1.45, color: colors.errorText }}>{pdfGenerateError}</div>
                  </div>
                )}
              </div>
            )}
          ...
          {lastGenerationTime != null && !pdfGenerateError && (
            <div
              style={{
                padding: "10px 12px",
