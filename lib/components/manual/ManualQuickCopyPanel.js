import QuickCopyTileIcon from "../shared/QuickCopyTileIcon";
...
export function QuickCopyGrid({ colors, quickCopyFields, copiedField, copyToClipboard }) {
  ...
  return (
    ...
    >
      {quickCopyFields.map(({ key, label, value, icon, iconUrl, iconRender, openInNewTab }) => (
        <button
          key={key}
          type="button"
          onClick={() => {
        ...
        >
          <QuickCopyTileIcon iconUrl={iconUrl} iconRender={iconRender} icon={icon} color={colors.text} />
          <span
            style={{
              fontSize: 10,
              fontWeight: "600",
