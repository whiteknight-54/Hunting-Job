  ...
  return (
    <div style={cardStyle}>
      ...
      <button type="button" onClick={handleGenerate} disabled={!canSubmit} style={{ ...primaryBtn(!canSubmit), width: "100%", marginBottom: 10 }}>
        {getResumeActionLabel({ busy: disable, variant: "auto", aiModel })}
      </button>
      ...
      {(disable || pdfRunFailed) && (
        <div style={{ marginBottom: 14 }}>
          <PdfRunProgressStatus
            ...
            aiModel={aiModel}
            failed={pdfRunFailed && !disable}
            failedTitle="Generation failed"
            failedAfterSeconds={pdfFailedElapsed}
            errorMessage={generateError}
          />
          {disable && (
            <div style={{ fontSize: 12, lineHeight: 1.45, color: colors.text, opacity: 0.75, marginTop: 4 }}>
      ...
      {lastPdfDownloadSeconds != null && !pdfRunFailed && (
