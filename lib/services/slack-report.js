...
function formatDriveUploadLine(driveUpload) {
  if (!driveUpload || driveUpload.skipped) return null;
  if (driveUpload.ok) {
    const link = driveUpload.webViewLink ? ` <${driveUpload.webViewLink}|Open in Drive>` : "";
    return `Drive: *uploaded*${link}`;
  }
  if (driveUpload.error) {
    return `Drive: *upload failed* — ${String(driveUpload.error).slice(0, 200)}`;
  }
  return null;
}
 ...
 */
export async function sendSlackPdfSuccessReport({ fileName, aiAgent, promptId, jd, driveUpload }) {
  const jdBlock = formatJdPreview(jd).replace(/```/g, "'''");
  const driveLine = formatDriveUploadLine(driveUpload);
  const messageText = [
    `*PDF generated*  ${fileName}`,
    `AI Agent: *${aiAgent}*, Prompt: *${promptId || "—"}*`,
    ...(driveLine ? [driveLine] : []),
    "```",
    jdBlock,
    "```",
