...
function formatDriveUploadInline(driveUpload) {
  if (!driveUpload || driveUpload.skipped) return "Drive: *download*";
  if (driveUpload.ok) {
    if (driveUpload.webViewLink) {
      return `Drive: <${driveUpload.webViewLink}|uploaded>`;
    }
    return "Drive: *uploaded*";
  }
  if (driveUpload.error) {
    return `Drive: *failed* — ${String(driveUpload.error).slice(0, 120)}`;
  }
  return "Drive: —";
}

/** Build Slack message body for a successful PDF run. */
export function formatPdfSlackReport({ fileName, aiAgent, promptId, jd, driveUpload, userName }) {
  const jdBlock = formatJdPreview(jd).replace(/```/g, "'''");
  const who = String(userName || "").trim() || "User";
  const meta = `AI: *${aiAgent || "—"}* · Prompt: *${promptId || "—"}* · ${formatDriveUploadInline(driveUpload)}`;
  return [`*${who} generate*  ${fileName}`, meta, "```", jdBlock, "```"].join("\n");
}

/**
 * Posts a success-only PDF summary to Slack (bot token preferred, else Incoming Webhook).
 * @param {string} [userName] — Slack display name from signed-in session
 * @param {object} [driveUpload] — optional result from uploadPdfToGoogleDrive
 */
export async function sendSlackPdfSuccessReport({ fileName, aiAgent, promptId, jd, driveUpload, userName }) {
  const messageText = formatPdfSlackReport({ fileName, aiAgent, promptId, jd, driveUpload, userName });
