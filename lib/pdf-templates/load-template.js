...
/** Resolved React-PDF components per warm lambda (avoids re-import on each PDF). */
...
export async function getTemplateAsync(templateId) {
  ...
  if (resolvedTemplates.has(name)) return resolvedTemplates.get(name);

  const load = TEMPLATE_LOADERS[name];
  if (!load) return null;
  try {
    const component = await load();
    if (component) resolvedTemplates.set(name, component);
    return component;
  } catch {
    return null;
  }
