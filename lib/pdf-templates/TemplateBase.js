...
import { CenterContactRow, SplitContactList } from './ContactHeader';

// Section title visual styles: bar (full bg), box (left border + bg), line (underline only), minimal (text only)
const buildSectionTitleStyle = (sectionTitleStyle, colors, fonts) => {
    const base = {
    ...
    const primaryDark = colors.primaryDark || '#1e40af';
    const sectionBg = colors.sectionBg || '#eff6ff';
    switch (sectionTitleStyle) {
        case 'box':
            return { ...base, backgroundColor: sectionBg, padding: '6px 12px', borderLeftWidth: 4, borderLeftColor: primary };
        case 'line':
            return { ...base, borderBottomWidth: 2, borderBottomColor: primaryDark, paddingBottom: 4, backgroundColor: 'transparent' };
        case 'minimal':
            return { ...base, fontSize: 9, color: colors.textLight || '#64748b', backgroundColor: 'transparent', letterSpacing: 0.5 };
        case 'bar':
        default:
            return { ...base, backgroundColor: sectionBg, padding: '8px 15px', borderLeftWidth: 3, borderLeftColor: primary };
    }
};
...
export const createResumeTemplate = (config) => {
    const {
        ...
        sectionTitleStyle = 'bar', // 'bar' | 'box' | 'line' | 'minimal'
    } = config;

    const sectionTitleBuilt = buildSectionTitleStyle(sectionTitleStyle, colors, fonts);
    ...
    const styles = StyleSheet.create({
        ...
        header: {
            ...
            marginBottom: 14,
            paddingBottom: colors.headerBg ? 0 : 10,
            borderBottomWidth: colors.headerBg ? 0 : 2,
        ...
        sectionTitle: sectionTitleBuilt,
        summary: {
    ...
    const TemplateComponent = ({ data }) => {
        ...
        const linkColor = colors.primary || '#2563eb';
        ...
        const renderHeader = () => {
            ...
            return (
                <View style={styles.header}>
                    <Text style={styles.name}>{name}</Text>
                    {title && <Text style={styles.title}>{title}</Text>}
                    <CenterContactRow data={data} textStyle={styles.contact} linkColor={linkColor} />
                </View>
            );
        };
